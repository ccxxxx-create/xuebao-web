...
self.addEventListener("activate", function (e) {
  e.waitUntil(Promise.all([
    self.clients.claim(),
    caches.keys().then(function (ks) { return Promise.all(ks.filter(function (k) { return k.indexOf("xuebao-shell-v") === 0 && k !== "xuebao-shell-v22"; }).map(function (k) { return caches.delete(k); })); })
  ]));
});
  ...
  if (url.pathname.endsWith("/update.json")) {
    e.respondWith(
      fetch(req, { cache: "no-store" }).then(function (r) {
        if (r && r.ok) {
          var copy = r.clone();
          caches.open("xuebao-shell-v22").then(function (c) { c.put(req, copy); });
        }
        return r;
      }).catch(function () {
