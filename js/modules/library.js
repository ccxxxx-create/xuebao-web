...
(function () {
  ...
  var state = { q: "", channel: "", trans: "", from: "", to: "", sort: "time", page: 1, openUrl: null, zhTab: "zh" };
  var running = {};
  ...
  var M = {
    ...
    async render(el) {
      ...
      function listHtml(list, terms) {
        ...
        var html = slice.map(function (a) { return cardHtml(a, terms); }).join("");
        if (pages > 1) {
          html += '<div class="pager">' +
            '<button class="btn sm" data-act="pg" data-pg="' + (state.page - 1) + '"' + (state.page <= 1 ? " disabled" : "") + ">上一页</button>" +
            '<span class="pg-info">第 ' + state.page + " / " + pages + " 页 · 共 " + list.length + " 篇</span>" +
            '<button class="btn sm" data-act="pg" data-pg="' + (state.page + 1) + '"' + (state.page >= pages ? " disabled" : "") + ">下一页</button>" +
            "</div>";
        }
        return html;
      }
      ...
      function detailHtml(a, terms, run) {
        ...
        return '<div class="detail-body">' +
          '<div class="art-meta" style="margin-top:0">' +
          ...
          "</div>" +
          (t === "zh"
            ? '<div class="field"><label>中文摘要' + (a.summaryZh ? "" : "（未生成，点卡片「生成标题+摘要」）") + "</label>" +
          ...
          '<div class="art-actions" style="margin-top:12px"><a class="btn sm" href="' + H.esc(a.url) + '" target="_blank" rel="noopener">查看原文（打开原网站）→</a></div>' +
          "</div>";
      }
      function bind(el, all, terms) {
        ...
        el.querySelector("#fCh").addEventListener("change", function (e) { state.channel = e.target.value; state.page = 1; App.refresh(); });
        el.querySelector("#fTr").addEventListener("change", function (e) { state.trans = e.target.value; state.page = 1; App.refresh(); });
        el.querySelector("#fSort").addEventListener("change", function (e) { state.sort = e.target.value; App.refresh(); });
        el.querySelector("#fFrom").addEventListener("change", function (e) { state.from = e.target.value; state.page = 1; App.refresh(); });
        el.querySelector("#fTo").addEventListener("change", function (e) { state.to = e.target.value; state.page = 1; App.refresh(); });
        ...
        if (!el.__wbClick) {
          el.__wbClick = true; // 容器级监听只绑定一次，避免重复 refresh 造成事件叠加
          el.addEventListener("click", function (e) {
            ...
            if (act === "open") { state.openUrl = state.openUrl === url ? null : url; App.refresh(); }
              ...
              var p = parseInt(btn.dataset.pg, 10);
              if (p >= 1) { state.page = p; App.refresh(); }
            }
