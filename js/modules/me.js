/* modules/me.js —— 「我的」（移动端聚合页）：仅手机端底部导航最右入口。
   聚合 术语库/喜好/设置 等中低频功能；排行榜/信源走总览页频道 tab，本页不再重复。 */
(function () {
  "use strict";
  var ENTRIES = [
    { key: "terms", label: "术语库", icon: '<svg viewBox="0 0 24 24"><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/></svg>', desc: "内置军语库 · 可搜索浏览" },
  ...
  var M = {
    ...
    async render(el) {
      ...
      el.innerHTML =
        ...
        '<div class="card" style="background:linear-gradient(135deg,var(--primary),var(--accent));color:var(--on-primary);border:0">' +
        '<div class="art-head"><div class="brand-mark" style="background:color-mix(in srgb,var(--on-primary) 22%,transparent)">势</div>' +
        ...
        '<div class="me-list">' +
        // v1.24：排行榜/信源不再放「我的」——总览页频道 tab 已有，避免双入口
        ENTRIES.map(function (e) { return meRow(e.key, e.label, e.label, e.desc, e.icon); }).join("") +
        "</div></div>" +
